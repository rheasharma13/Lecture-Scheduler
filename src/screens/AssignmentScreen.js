...
const [assignmentSubmissionDate,setAssignmentSubmissionDate]=useState("2001-11-02");
...
// console.log("File",file.name)
    storage.ref('assignment_files/' + id + '_' + assignmentNumber + '/' + file.name).put(file).on('state_changed',
        (fileSnapshot) => {
          // console.log("Snapshot",fileSnapshot.ref.fullPath)
            let percent = (fileSnapshot.bytesTransferred / fileSnapshot.totalBytes) * 100;
  ...
  return (
    <div className="class">
      ...
      {user?.uid==classData.creatorUid?<div className="class__announce">
     <h2>Create an Assignment</h2>
      ...
      /> */}
        ...
        <div style={{display: "flex","margin-left": "20px",padding: "15px 20px","margin-right": "20px","justify-content":"space-between"}}>
        
        <input type="file"  onChange={(e)=>{setFile(e.target.files[0]); }}/>
 
 <button variant="contained"  style={{"margin-right": "auto" , display:"flex"}}onClick={(e)=> uploadFile(e)}> Upload File </button>
        </div>
        <Button variant="contained"  style={{"margin-left": "auto" , "display": "block", "background-color":"green" , color:"white"}} onClick={createAssignment}>
          Submit
        </Button>
      </div>:""}
